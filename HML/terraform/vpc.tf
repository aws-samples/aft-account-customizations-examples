...
resource "aws_vpc" "vpc" {
  #checkov:skip=CKV2_AWS_11: No need HML environment.
  #checkov:skip=CKV2_AWS_12: Already done.
  cidr_block = local.cidr
  tags = {
    Name = "${data.aws_ssm_parameter.project.value}-HML"
  }
}
...
resource "aws_internet_gateway" "gw" {
  ...
  tags = {
    Name = "${data.aws_ssm_parameter.project.value}-HML-public-subnet"
  }
}
...
resource "aws_nat_gateway" "ngw" {
  ...
  tags = {
    Name = "${data.aws_ssm_parameter.project.value}-HML-gw-NAT"
  }
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name        = "${data.aws_ssm_parameter.project.value}-HML-private-subnet"
  }
...
resource "aws_route_table" "main-public" {
  ...
  tags = {
    Name        = "${data.aws_ssm_parameter.project.value}-HML-public-route"
  }
...
resource "aws_route_table" "main-private" {
  ...
  tags = {
    Name        = "${data.aws_ssm_parameter.project.value}-HML-private-route"
  }
  depends_on = [ aws_nat_gateway.ngw ]
